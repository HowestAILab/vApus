...
namespace vApus.Monitor
{
    public class MonitorControl : DataGridView
    {
        ...
        public void Init(Dictionary<Entity, List<CounterInfo>> wiw, string[] units)
        {
            ...
            var clms = new DataGridViewColumn[_headers.Length];
                ...
                lock (_lock)
                {
                    ...
                    if (monitorValues.Length != _headers.Length)
                        LogWrapper.LogByLevel(
                            "[Monitoring] The number of monitor values is not the same as the number of headers!\nThis is a serious problem.",
                            LogLevel.Error);
                }
            if (_keepAtEnd)
            {
        ...
        public void Save(string fileName)
        {
            ...
            {
                MessageBox.Show("Cannot access '" + fileName + "' because it is in use!", string.Empty,
