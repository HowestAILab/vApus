...
namespace vApus.Stresstest
{
    ...
    public class Log : LabeledBaseItem
    {
        ...
        public void GetUniqueParameterTokenDelimiters(out string beginTokenDelimiter, out string endTokenDelimiter, out bool warning, out bool error, bool autoNextOnError = true)
        {
            ...
            foreach (LogEntry logEntry in GetAllLogEntries())
            {
                ...
                int i = logEntry.GetUniqueParameterTokenDelimiters(autoNextOnError, out b, out e, out warn, out err, _preferredTokenDelimiterIndex);
        ...
        public IEnumerable<LogEntry> GetAllLogEntries()
        {
            lock (_lock)
            {
                foreach (var item in this)
                {
                    ...
                    else foreach (LogEntry childItem in item)
        ...
        public void SynchronizeTokens(Dictionary<BaseParameter, KeyValuePair<int, int>> oldAndNewIndices, KeyValuePair<string, string> oldAndNewBeginTokenDelimiter, KeyValuePair<string, string> oldAndNewEndTokenDelimiter)
        {
            //Synchronize only if needed.
            if (oldAndNewIndices.Count == 0)
                return;

            Dictionary<string, string> oldAndNewTokens = new Dictionary<string, string>();
        ...
        public string ReplaceTokenDelimitersInLogEntryStringAsImported(string beginTokenDelimiter, string endTokenDelimiter)
        {
        ...
        Replace:
            ...
            foreach (LogEntry entry in GetAllLogEntries())
                entry.LogEntryStringAsImported = entry.LogEntryStringAsImported.Replace(beginTokenDelimiter, begin).Replace(endTokenDelimiter, end);
